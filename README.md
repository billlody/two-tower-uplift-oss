...
|------|---------|---------------|---------------------|
| Confounding & balance | `python demos/confounding_balance.py` | §Offline Eval, balance table + FWL figure | **Yes.** Raw `corr(ad_load, watch_percent)` is spuriously **positive** (~+0.16); within-stratum it flips **negative** (~−0.30; effect persists, not zero); confounder `|SMD|` shrinks from ~1.9 to <0.1. Matches the production Databricks notebook. |
